<form method="POST" enctype="multipart/form-data">
    ...
    <select name="kategori_item_ids[]" class="form-control">
        <option @if ($selected == '') selected @endif value="">--Pilih--</option>
        @foreach($kategoriItems as $kategori)
            <option value="{{ $kategori->id }}"
                @if(in_array($kategori->id, $selectedKategori)) selected @endif>
    ...
    <div class="form-group">
        ...
        <input type="file" name="foto" class="form-control" {{ $method == 'new' ? 'required' : '' }}
            value="{{ $item->foto ?? '' }}">

        @if (!empty($item['foto']))
            <img src="{{ asset('storage/item/' . $item['foto']) }}" width="100" alt="Foto">
        @endif
    </div>
